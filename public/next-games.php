...
function pageController()
{
    ...
    if (Input::has('league')) {
        ...
        $league = Input::has('league');
        $sql .= "WHERE visitor_team_id IN (SELECT id FROM teams WHERE league = '$league') OR local_team_id IN (SELECT id FROM teams WHERE league = '$league')";
    }
    if (Input::has('team')) {
        // Concatenate the WHERE part to retrieve only the games for teams with
        // a name similar to the one provided by the user.
        $team = Input::has('team');
        $sql .= "WHERE visitor_team_id IN (SELECT id FROM teams WHERE name LIKE '%$team%') OR local_team_id IN (SELECT id FROM teams WHERE name LIKE '%$team%')";
    }
